  ...
  departments.forEach((dept) => {
    years.forEach((year) => {
      for (let i = 0; i < 10; i++) {
        ...
        data.push({ rank: rankStr, name: `${lastName} ${firstName} ${lastName[0]}.`, dept: dept, year: year, score: `${cgpaScores[i]} CGPA` });
      }
    });
  });
  return data;
...
export default function Top10Page() {
  ...
  const filteredRankings = useMemo(() => {
    return allTop10.filter(row => row.dept === selectedDept && row.year === selectedYear && row.name.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <main style={{ minHeight: "100vh", background: "var(--slate-50)" }}>
        <section className="hero-fullscreen" id="home" style={{ minHeight: "50vh", height: "50vh", background: "#4b5563" }}>
          <div className="hero-bg-image">
            <Image src="/assets/hero/hero-section.png" alt="Toppers Banner" width={1400} height={700} className="hero-bg-img" priority />
          </div>
          ...
          <div className="hero-content container" style={{ paddingTop: "100px", display: "flex", flexDirection: "column", justifyContent: "flex-end", height: "100%", paddingBottom: "80px" }}>
            <h1 className="hero-main-title" style={{ paddingRight: "15px", color: "#fff", margin: "0 0 1rem 0" }}><em>Top-10</em></h1>
          ...
          <div className="container" style={{ maxWidth: "1000px" }}>
            <div className={styles.headerRow}>
              <div className={styles.headerText}>
                <h3 className={styles.sectionTitle} style={{ margin: 0 }}>Top-10 <span style={{ color: "#8a0000" }}>Rankers</span></h3>
                <p className={styles.sectionSubtitle} style={{ marginTop: "0.25rem" }}>Browse leading scorers by department and graduation year</p>
              </div>
              ...
              <>
                <div className={styles.leaderboardCard}>
                  {filteredRankings.map((row, idx) => (
                    <div key={idx} className={`${styles.leaderboardRow} ${styles.mobileClickable}`} onClick={() => handleAvatarClick(idx, row)}>
                      <div className={styles.rankBadgeWrap}>
                        <span className={`${styles.rankBadge} ${getRankClass(row.rank)}`}>{row.rank.replace(/(st|nd|rd|th)/, '')}</span>
                      </div>
                      <div className={styles.mobileAvatar}>
                        <Image src={`/assets/toppers/${photoNames[idx]}.jpg`} alt={row.name} width={34} height={34} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      </div>
                  ...
                  <>
                    ...
                    <div className={styles.mobilePopup}>
                      <div className={styles.mobilePopupImg}>
                        <Image src={`/assets/toppers/${photoNames[activeCard.idx]}.jpg`} alt={activeCard.row.name} fill style={{ objectFit: 'cover' }} />
                      </div>
