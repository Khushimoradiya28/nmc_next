...
export default function CompositionOfIQAC() {
  ...
  const toggleFaculty = (index) => {
    setExpandedFaculty(prev => ({ ...prev, [index]: !prev[index] }));
  };

  const toggleExternal = (index) => {
    setExpandedExternal(prev => ({ ...prev, [index]: !prev[index] }));
  };
  return (
    <>
      <Header />
      <main className={styles.page}>
          ...
          <div className="hero-content container" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", height: "100%", paddingBottom: "45px" }}>
            ...
            <nav aria-label="breadcrumb">
              <ol style={{ display: "flex", listStyle: "none", padding: 0, margin: 0, gap: "0.5rem", color: "var(--gold-500, #f59e0b)", fontSize: "0.85rem", fontWeight: "600", textTransform: "uppercase", letterSpacing: "1px" }}>
                ...
                <li style={{ color: "rgba(255,255,255,0.5)" }}>&gt;</li>
                <li aria-current="page" style={{ color: "var(--gold-500, #f59e0b)" }}>Composition</li>
              </ol>
            </nav>
          </div>
        </section>
        ...
        <IQACAnimations>
          <div className={styles.container}>
            <div className={styles.sectionHeader} data-animate="header">
              <span className={styles.topBadge}>Executive Members</span>
            ...
            <div className={styles.leadersRow}>
              ...
              >
                <div className={styles.bgTextWatermark}>NMC</div>
                <div className={styles.cardHeader}>
                  <span className={styles.roleTagRed}>{leadership.title}</span>
                  <h3 className={styles.leaderName}>{leadership.name}</h3>
                  <p className={styles.leaderRole}>{leadership.role}</p>
                </div>
                <div className={styles.cardBody}>
                  <p className={styles.leaderDesc}>{leadership.desc}</p>
                </div>
              </motion.div>
              ...
              >
                <div className={styles.bgTextWatermark}>NMC</div>
                <div className={styles.cardHeader}>
                  <span className={styles.roleTagGold}>{coordinator.title}</span>
                  <h3 className={styles.leaderName}>{coordinator.name}</h3>
                  <p className={styles.leaderRole}>{coordinator.role}</p>
                </div>
                <div className={styles.cardBody}>
                  <p className={styles.leaderDesc}>{coordinator.desc}</p>
                </div>
              </motion.div>
            </div>
            ...
            <div style={{ position: 'relative', marginTop: '4rem' }}>
              ...
              <div className={styles.boardCard}>
                <div className={styles.bgTextWatermarkRight}>IQAC</div>
                <h4 className={styles.boardCardHeading}>Management &amp; Trustee</h4>
                <p className={styles.boardMemberName}>{management.name}</p>
                <p className={styles.boardMemberRole}>{management.role}</p>
                <p className={styles.boardMemberDesc}>{management.desc}</p>
              </div>

              {/* Faculty Members */}
              <div className={styles.boardCard}>
                <div className={styles.bgTextWatermarkRight}>IQAC</div>
                <h4 className={styles.boardCardHeading}>Senior Faculty Representatives</h4>
                  ...
                  {facultyMembers.map((m, i) => {
                    ...
                    return (
                      <div key={i} className={styles.listItem}>
                        ...
                        <div>
                          <p className={styles.listMemberName}>{m.name}</p>
                          <p className={styles.listMemberRole}>{m.role}</p>
                          <p className={styles.listMemberDesc}>
                            {isExpanded ? m.desc : shortDesc}
                            {m.desc.length > 75 && (
                              <button 
                                onClick={() => toggleFaculty(i)} 
              ...
              <div className={styles.boardCard}>
                <div className={styles.bgTextWatermarkRight}>IQAC</div>
                <h4 className={styles.boardCardHeading}>External &amp; Outreach Members</h4>
                <div className={styles.memberList}>
                  {externalMembers.map((m, i) => {
                    ...
                    return (
                      <div key={i} className={styles.listItem}>
                        <div className={styles.bulletGold} />
                        <div>
                          <p className={styles.listMemberName}>{m.name}</p>
                          <p className={styles.listMemberRole}>{m.role}</p>
                          <p className={styles.listMemberDesc}>
