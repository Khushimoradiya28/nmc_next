...
export const DateCard = ({ date, title = 'Created At', align = 'center' }) => {
  ...
  return (
    <div className="relative group inline-block">
      ...
      <div
        className={
          'absolute bottom-full mb-1.5 hidden group-hover:flex flex-col z-50 pointer-events-none ' +
          (align === 'right'
            ? 'right-0 items-end'
            : align === 'left'
            ? 'left-0 items-start'
      ...
      >
        ...
        <div
          className={
            'w-2 h-2 -mt-1 rotate-45 bg-gray-900 border-r border-b border-gray-700 ' +
...
const DateBox = ({ created_at, updated_at, createdAt, updatedAt, showUpdated = true }) => {
  const actualCreated = created_at || createdAt;
  const actualUpdated = updated_at || updatedAt || actualCreated;

  if (!showUpdated) {
    return <DateCard date={actualCreated} title="Submitted At" align="center" />;
  }
