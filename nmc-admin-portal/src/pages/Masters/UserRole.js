...
import UserRoleTable from '../../components/master/userrole/UserRoleTable';
import MainDrawer from '../../components/drawer/MainDrawer';
import UserRoleDrawer from '../../components/master/drawer/UserRoleDrawer';
import Breadcrumb from "../../components/form/Breadcrumb";

const UserRoles = () => {
  const { toggleDrawer, setIsUpdate } = useContext(SidebarContext);
  const [searchText, setSearchText] = useState("");
  const { data, loading } = useAsync(() => UserRoleServices.getAllUserRoles({ search: searchText }));
      ...
      {loading ? (
        ...
        <TableContainer className="rounded-b-lg">
          <Table>
            ...
            <UserRoleTable brand={dataTable} currentPage={currentPage} resultsPerPage={resultsPerPage}/>
          </Table>
          <TableFooter>
            <Pagination
