...
import CommodityTable from '../../components/master/commodity/CommodityTable';
import MainDrawer from '../../components/drawer/MainDrawer';
import CommodityDrawe from '../../components/master/drawer/CommodityDrawe';
import Breadcrumb from "../../components/form/Breadcrumb";

const UserRoles = () => {
  const { toggleDrawer, setIsUpdate } = useContext(SidebarContext);
  const [searchText, setSearchText] = useState("");
  const { data, loading } = useAsync(() => CommodityService.getAllData({ search: searchText }));
  // const { data, loading } = useAsync(CommodityService.getAllData);
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 my-2">
        ...
        <PageTitle>Commodities</PageTitle>
          ...
          <Table>
            <TableHeader>
              <tr>
                ...
                <TableCell className="text-center">Commodity</TableCell>
                {/* <TableCell>Status</TableCell> */}
                <TableCell className="text-center">Date</TableCell>
                <TableCell className="text-center">Actions</TableCell>
              </tr>
            </TableHeader>
            <CommodityTable brand={dataTable} currentPage={currentPage} resultsPerPage={resultsPerPage}/>
          </Table>
          <TableFooter>
            <Pagination
