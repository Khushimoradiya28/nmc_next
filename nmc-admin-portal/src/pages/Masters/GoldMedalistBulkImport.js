...
// Frontend origin where /assets/toppers images live (configurable, dev fallback :3000)
const FRONTEND_URL = (process.env.REACT_APP_FRONTEND_URL || 'http://localhost:3000').replace(/\/$/, '');

// Resolve a row's Evidence value into a browser-loadable preview URL
...
// CSV template columns (matches backend expected headers)
...
const TEMPLATE_SAMPLE = [
  '1,2015-16,B.A.,TYBA,PAREKH KHUSHBU,1ST,UNIVERSITY RANK HOLDER,',
];

// Phases: idle -> uploading -> auditing -> result
const GoldMedalistBulkImport = () => {
  ...
  const [report, setReport] = useState(null); // { summary, valid, invalid }
  ...
  // Animate the audit progress bar while awaiting backend response
  const runAuditAnimation = useCallback(() => {
    setAuditPercent(0);
    let pct = 0;
    const timer = setInterval(() => {
      // Ease towards 90% while waiting; final jump to 100 happens on response
  ...
  const handleValidate = async () => {
    if (!file) {
      notifyError('Please choose a CSV file first.');
      return;
    }
    try {
      setReport(null);
      ...
      const res = await GoldMedalistServices.bulkValidate(file, (pct) => {
        setUploadPercent(pct);
        if (pct >= 100) setPhase('auditing');
      });

      // Upload done -> run audit animation, then settle to 100 with the response
      setPhase('auditing');
      const auditTimer = runAuditAnimation();
      // Small delay so the auditing bar is visible even for tiny files
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      const res = await GoldMedalistServices.bulkImport(report.valid);
      setImportResult(res);
      setIsUpdate(true);
  ...
  const downloadTemplate = () => {
    ...
    a.href = url;
    a.download = 'gold-medalist-import-template.csv';
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 my-6">
        <div>
          <PageTitle>Gold Medalist — Bulk Import</PageTitle>
          <p className="text-xs text-gray-500 dark:text-gray-400 -mt-2">
            Upload a CSV to add multiple achievers at once. Data is audited before import.
          </p>
        </div>
        <div className="flex gap-3">
          <NavLink to="/master/gold-medalists">
                        ...
                        <tr key={item.row} className="border-t border-gray-100 dark:border-gray-700">
                          ...
                          <td className="px-4 py-2">
                            ...
                            <span
                              ...
                              style={{ display: evUrl ? 'none' : 'inline' }}
                            >
                          ...
                          <td className="px-4 py-2 text-gray-600 dark:text-gray-300">{item.data?.subCourse}</td>
                          <td className="px-4 py-2 text-gray-600 dark:text-gray-300">{item.data?.academicYear}</td>
                          <td className="px-4 py-2 text-gray-600 dark:text-gray-300">{item.data?.rank}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
export default GoldMedalistBulkImport;
